...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	function excluir(id) {
		...
		$.get(contexto + "/regra/confirmaExcluir/"+id, null, 
		function(data) {
  				...
  				<div class="row">
    				<div class="span15">
    				  <div class="well" style="padding: 10px 19px;">
       					 <a href="<c:url value="/regra/form" />" class="btn primary">Inserir</a>
      				  </div>
				        ...
				        <tbody>
				        	<c:if test="${not empty regraList}">						
								<c:forEach items="${regraList}" var="regra">
									<tr>
										<td><a href="<c:url value="/regra/alterar/${regra.id}" />">${regra.nome}</a></td>
						            	<td>${regra.parceiro.nome}</td>
						            	<td>${regra.la.nome}</td>
						            	<td>${regra.keycode.nome}</td>
						            	<td>${regra.sharecodeFormat}</td>
						            	<td>${regra.ativoFormat}</td>
						            	<td><a href="#" onclick="javascript:excluir(${regra.id})" >Excluir</a></td>
						          	</tr>
								</c:forEach>				
							</c:if>
				        </tbody>
